function graph_data = vec_to_graph(vs)
    ...
    graph_data.A_list = cell(size(vs,2),1);
    for k=1:size(vs,2)
        ...
        for ii=1:L %sadly do need to loop over this twice - otherwise A is generated incorrectly. Would be great fun to come up with a more efficient method.
            trace_sys = systems;
            trace_sys(ii) = [];
%             red_ii = TrC(rho, trace_sys,dims);
%             Entropy(red_ii);
            ent_list(ii) = Entropy(TrC(rho, trace_sys,dims));
        end
        for ii=1:L
            for jj = ii+1:L
               trace_pair = systems;
               trace_pair(ii) =[];
               trace_pair(jj-1) = [];
               A_temp(ii,jj) =  abs(Entropy(TrC(rho,trace_pair,dims)));
%                fprintf('S_AB %f' ,A(ii,jj))
        ...
        graph_data.A_list{k} = A; %Aleph - has on-site entropy on diagonal. Can recover G (zero diag) easy)
    end
...
%                trace_pair(ii) =[];
%                trace_pair(jj-1) = [];
%                subsys  = TrC(rho,trace_pair,dims);
