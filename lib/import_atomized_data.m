...
%             network_data.A.unif_projection = sum(sum(Aleph_UD));
            ...
            % Llacian properties
            L_offdiag = A_temp + A_temp';
            L_diag = sum(L_offdiag);
            network_data.L.Laplacian = diag(L_diag)-L_offdiag;    
            ...
            % Graph properties
            D_temp = sum(L_offdiag)';
            mu_temp =L_offdiag*D_temp./D_temp; % Weighted sum of (normalized) neighbour degrees
            norm = zeros(data.L,1);
            for i=1:data.L
            ...
            inf_grid = L_diag + L_offdiag;
            % QMI decay
            QMI_grid = zeros(data.L);
            for ii = 1:data.L
                QMI_grid(ii,:) = circshift(L_offdiag(ii,:),-ii+1);
            end
