...
function plot_L_var(loop_data,config)
    for field_idx=1:length(config.fieldnames)
            ...
            SJ_var_curves = cellfun(@(x) x.data.G{field_idx}.SJ_integrated, loop_data,'UniformOutput',false);
            Ent_var_curves = cellfun(@(x) x.data.G{field_idx}.entropy, loop_data,'UniformOutput',false);
            ...
            subplot(1,2,1)
            for N = 1:num_Ls
            xvals = loop_data{N}.conf.G{field_idx}.viz.W_list; 
            [~,xidx] = sort(xvals);
            plot(xvals(xidx),SJ_var_curves{N},'x-','Color',cm(N,:))
            hold on
