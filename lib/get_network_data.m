...
%   struct network_data with fields defined below, indexed by {realization#, eigenvalue}
    ...
    network_data.L.Laplacian = zeros(kmax,data.num_eigs,data.L,data.L);
    network_data.L.evals = zeros(kmax,data.num_eigs,data.L);
    network_data.L.evecs = zeros(kmax,data.num_eigs,data.L,data.L);
   
    % Aleph properties
    network_data.A.Aleph = zeros(kmax,data.num_eigs,data.L,data.L);
    network_data.A.evals = zeros(kmax,data.num_eigs,data.L);
    network_data.A.evecs = zeros(kmax,data.num_eigs,data.L,data.L);
    network_data.A.trace = zeros(kmax,data.num_eigs,data.L);
%     network_data.
    
    % Graph properties
    network_data.G.degree_list = zeros(kmax,data.num_eigs,data.L);
    network_data.G.weight_list = zeros(kmax,data.num_eigs,data.L*(data.L-1)/2);
    network_data.G.node_centrality = zeros(kmax,data.num_eigs,data.L);
    
    % Physical properties
    network_data.P.entropy_VN=zeros(kmax,data.num_eigs,data.L);
    network_data.P.nrg = zeros(kmax,data.num_eigs);
