...
%% First item: degree distribution
...
config.viz.log_hist_win = [1,10];
degree_viz = distribution_viz(degree_dist,'degree distribution',config);
...
suptitle('Scaled degree distribution fits')

subplot(4,3,1)
for N=1:Nmax
    plot(degree_viz{N}.log_hist_bins,degree_viz{N}.log_hist_counts,'-','Color',cm(N,:));
    hold on
    xlabel('-log10(X)')
    ylabel('P(x=X)')
  
    IC = [2,4.5];
    betafits{N} = fitnlm(degree_viz{N}.log_hist_bins,degree_viz{N}.log_hist_counts,beta_func,IC);
    plot(degree_viz{N}.log_hist_bins,degree_viz{N}.log_hist_counts,'-','Color',cm(N,:));
    hold on
...
degree_viz_unscaled = distribution_viz(degree_dist,'degree distribution',config);

X = linspace(0,1,100);
subplot(4,3,2)
for N=1:Nmax
    plot(degree_viz_unscaled{N}.log_hist_bins,degree_viz_unscaled{N}.log_hist_counts,'-','Color',cm(N,:));
    hold on
    xlabel('-log10(X)')
    ylabel('P(x=X)')
    IC = [2.5,3];

%     weibfits{N} = fitnlm(degree_viz_unscaled{N}.log_hist_bins,degree_viz_unscaled{N}.log_hist_counts,wdist,IC);
    plot(degree_viz{N}.log_hist_bins,degree_viz{N}.log_hist_counts,'-','Color',cm(N,:));
...
%     u_weibfits{N} = fitnlm(degree_viz_unscaled{N}.log_hist_bins,degree_viz_unscaled{N}.log_hist_counts,wdist,IC);
%     plot(degree_viz{N}.log_hist_bins,degree_viz{N}.log_hist_counts,'-','Color',cm(N,:));
...
subplot(4,3,6)
for N=1:Nmax
[f,x] = ecdf(degree_viz_unscaled{N}.log_dat);
plot(x,f)
hold on
g = cdf('wbl',X,ls(N),ks(N));
plot(X,g)
